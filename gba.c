...
//      }
//  };
    //volatile u16 *dst = (videoBuffer + OFFSET(row, col, WIDTH));
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  for (int i = 0; i < height; i++){
      ...
      DMA[DMA_CHANNEL_3].dst = videoBuffer + OFFSET(row, col, WIDTH) + OFFSET(i, 0, WIDTH);
...
void undrawImageDMA(int row, int col, int width, int height, const u16 *image) {
    for (int i = 0; i < height; i++){
        //DMA[DMA_CHANNEL_3].src = image + width * i + OFFSET(row, col, WIDTH);
